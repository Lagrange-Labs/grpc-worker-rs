...
impl TestGateway {
    ...
    pub async fn run(
        ...
        tokio::spawn(async move {
            ...
            {
                panic!("error serving gw: {:?}", e);
            };
        });
        Ok((from_worker, to_worker))
    }
...
impl protobuf::workers_service_server::WorkersService for TestGateway {
    ...
    async fn worker_to_gw(
        ...
        let from_worker = self.msg_from_worker.clone();
...
pub fn new_uuid() -> protobuf::Uuid {
    let v = thread_rng().r#gen::<[u8; UUID_SIZE]>();
    protobuf::Uuid { id: v.to_vec() }
}
...
mod grpc_test {
    ...
    async fn test_grpc_dummy_gateway() -> anyhow::Result<()> {
        ...
        let connect_uri = format!("http://{}", listen_address);
        ...
        let (mut from_gw, to_gw) = worker_config.connect().await?;

        // first check if worker ready is being sent
        // expect to see task output
        let msg_from_worker =
            async_std::future::timeout(std::time::Duration::from_millis(1000), from_worker.recv())
                .await?;
        let Ok(WorkerToGwRequest {
            request: Some(worker_to_gw_request::Request::WorkerReady(ready)),
        }) = msg_from_worker
        else {
            bail!("unregonized worker ready msg: {:?}", msg_from_worker);
        };
        assert_eq!(ready.version, worker_config.version);
        assert_eq!(ready.worker_class, worker_config.class);
        ...
        // expect to see task
        let recv_todo =
            async_std::future::timeout(std::time::Duration::from_millis(1000), from_gw.recv())
                .await?;
        ...
        assert_eq!(todo_sent, todo_received);
        assert_eq!(task_id, uuid_sent);
        // send first reply
        let reply_sent = "this is big reply";
        to_gw
            .send(crate::grpc::protobuf::WorkerToGwRequest {
                request: Some(worker_to_gw_request::Request::WorkerDone(
        ...
        let recv_output =
            async_std::future::timeout(std::time::Duration::from_millis(1000), from_worker.recv())
                .await?;
        let Ok(WorkerToGwRequest {
            request: Some(worker_to_gw_request::Request::WorkerDone(WorkerDone { reply, task_id })),
        }) = recv_output
        else {
            bail!("unregonized reply task: {:?}", recv_output);
        };
        let Reply::ReplyString(recv_output) = reply.expect("no reply message?") else {
            bail!("unregonized reply string");
        };
        assert_eq!(reply_sent, recv_output);
        assert_eq!(uuid_sent, task_id);
